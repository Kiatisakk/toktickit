...
The existing index on requester and creation date is retained for My Tickets. The staff queue adds an index supporting its default ordering, and the comment and note tables are indexed by ticket. Sessions are indexed by user.
...
*This ran as described.* The bootstrap is `server/prisma/bootstrap-passwords.ts`; the two migrations are `add_session_and_password` and `password_hash_not_null`. Applying them to a populated database without the step between them fails with `column "passwordHash" of relation "User" contains null values` — which is what happened to the test database, on purpose, before it was recovered with `prisma migrate resolve --rolled-back`, bootstrapped and re-applied. A database created after this sprint never sees the intermediate state, because both migrations run against an empty table. Because Lab 2 named the model `User` rather than `RequesterUser` and defined all three roles up front, no identifier changes and no foreign key is rewritten.
