...
| UI-14 | FR-04 | Shell shows identity | Name and role badge shown; Logout present; no Change Requester action | `client/tests/lab-03/AppShell.test.tsx` | Partial — name, role and Logout pass. *No Change Requester action* cannot be true yet: both identity mechanisms coexist by design for one ticket, so the shell names the signed-in user and, when they differ, who is being acted as. Completed by the ticket that deletes the selector. |
