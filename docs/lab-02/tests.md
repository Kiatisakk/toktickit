...
| STYLE-08 | AC-25 | Table and cards carry the same values | Every column the figure shows is present as a header and reachable from the mobile card; the current sort is announced with `aria-sort`; both link to the same detail screen | `client/tests/lab-02/style/ticket-table.test.tsx` | **Pass** |
