...
| --- | --- | --- | --- | --- | --- |
| UNIT-01 | AC-09, BR-04 | Ticket number generator output shape | Matches `TKT-<4 digits>-<6 digits>`, including the exact values both labsheet figures show | `server/tests/lab-02/ticket-number.test.ts` | **Pass** |
...
| API-05 | AC-10, BR-18 | Field validation | Each rule returns `400 VALIDATION_FAILED` with `details` naming the field; no row created; a retired category is rejected; no ticket number is burned | `server/tests/lab-02/create-ticket.api.test.ts` | **Pass** |
...
| UI-07 | AC-08 | Reference data source | Options render from the mocked API; the classification fields stay disabled until they load; a load failure has its own state | `client/tests/lab-02/CreateTicket.test.tsx` | **Pass** |
| UI-08 | AC-10 | Field-level messages | Each message renders inside its own field group; whitespace-only counts as empty; the API is never called for an invalid form | `client/tests/lab-02/CreateTicket.test.tsx` | **Pass** |
...
| --- | --- | --- | --- | --- |
| Server (unit + API) | 9 | 108 | 108 | 2026-08-29 (Issue #17) |
| Client (component + style) | 10 | 100 | 100 | 2026-08-29 (Issue #17) |
| End-to-end | — | — | — | — |
