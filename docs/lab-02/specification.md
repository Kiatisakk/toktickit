...
`TKT-2025-001234` is the format both labsheet figures show, and the My Tickets figure
shows a contiguous run, which means a sequence rather than a random token.

The counter lives in its own table and is incremented with a single atomic statement
inside the same transaction that inserts the ticket. Reading the current maximum and
adding one would be a race: two simultaneous submissions would read the same value. The
unique constraint on the column is the backstop that turns any residual race into a
failed insert rather than a duplicate number.
