...
import { openMyTickets, shoot } from "./support";
 ...
 * pagination cannot appear on a list of three tickets.
 *
 * Lab 2's Development Requester Selection screen was photographed here too. The
 * screen was deleted in Lab 3 (BR-41), so its test went with it; the committed
 * Lab 2 screenshots remain as the record of what was submitted.
 *
...
test.describe("My Tickets controls doing something — §14 Part 7", () => {
  test("filters, sorting and pagination", async ({ page }, info) => {
    ...
    //
    // `page.request` rather than the `request` fixture: it shares the page's
    // cookie jar, so these calls carry Requester A's session exactly as the
    // screen's own do. It inherits the config's baseURL, which is the Vite dev
    // server (5173) — and Vite has no /api proxy, so a relative "/api/…"
    // returns index.html instead of JSON. The API lives on :3000, so these
    // calls use an absolute URL. Categories and related-systems answer with a
    // plain array (api-spec.md §3), not a { data } envelope.
